...
-- Run this entire script in your Cloud SQL Editor ONCE.
-- =========================================================
...
CREATE OR REPLACE VIEW `vw_cash_flow`  AS SELECT `tbl_cash_flow`.`entry_date` AS `entry_date`, `tbl_cash_flow`.`entry_type` AS `entry_type`, sum(`tbl_cash_flow`.`amount`) AS `total_amount` FROM `tbl_cash_flow` GROUP BY `tbl_cash_flow`.`entry_date`, `tbl_cash_flow`.`entry_type` ;
...
CREATE OR REPLACE VIEW `vw_marketing_roas`  AS SELECT `m`.`campaign_code` AS `campaign_code`, `m`.`campaign_name` AS `campaign_name`, `m`.`spend_amount` AS `spend_amount`, `m`.`revenue_amount` AS `revenue_amount`, `m`.`orders_generated` AS `orders_generated`, `m`.`roas` AS `roas` FROM `tbl_marketing_campaigns` AS `m` ;
...
CREATE OR REPLACE VIEW `vw_monthly_revenue`  AS SELECT date_format(`tbl_orders`.`order_date`,'%Y-%m') AS `revenue_month`, sum(`tbl_orders`.`grand_total`) AS `revenue` FROM `tbl_orders` WHERE `tbl_orders`.`deleted_at` is null GROUP BY date_format(`tbl_orders`.`order_date`,'%Y-%m') ;
...
CREATE OR REPLACE VIEW `vw_sales_trend`  AS SELECT date_format(`tbl_orders`.`order_date`,'%Y-%m') AS `sales_month`, sum(`tbl_orders`.`grand_total`) AS `sales` FROM `tbl_orders` GROUP BY date_format(`tbl_orders`.`order_date`,'%Y-%m') ;
...
-- =========================================================
-- APPLY SYSTEM PATCHES & FIXED STORED PROCEDURES
-- =========================================================
DROP PROCEDURE IF EXISTS sp_generate_business_code;
...
-- =========================================================
-- SEED INITIAL SYSTEM DATA (WAREHOUSE & ADMIN USER)
-- =========================================================
...
VALUES ('USR000001', 'Administrator', 'admin@hazel.com', '$2b$10$AIKJZbA/tVh1gwXqrR9YjeQJ3.rlhc1qE60ITK96kSnBnGKr3Q9Ji', 'admin', 1, 'system')
